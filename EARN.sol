...
contract EARNS is ERC20, Ownable {
    ...
    address public contract5;

    constructor() ERC20("EARNS", "EARN") Ownable(msg.sender) {
        _mint(address(this), TOTAL_SUPPLY);
    }

    function setContract4(address _contract4,address _contract5) external onlyOwner {
        contract4 = _contract4;
        contract5 = _contract5;
    }

    function renounceOwnership(uint256 soldSupply) external onlyOwner {
        require(soldSupply <= TOTAL_SUPPLY, "Sold supply exceeds total supply");
        uint256 unsoldSupply = TOTAL_SUPPLY - soldSupply;

        if (unsoldSupply > 0) {
            _transfer(address(this), contract5, unsoldSupply);
        }
        _transfer(address(this), contract4, soldSupply);
        renounceOwnership();
    }
